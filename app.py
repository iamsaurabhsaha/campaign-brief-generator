...
def render_brief_builder() -> None:
    ...
    # On first load, check URL for a step param (direct link / bookmark)
    if not st.session_state.get("_url_checked"):
        url_step = st.query_params.get("step", "")
        if url_step in _NAME_TO_STEP:
            st.session_state.wizard_step = _NAME_TO_STEP[url_step]
    ...
    # --- Step 2: Strategy ---
        ...
        if st.session_state.get("ai_background"):
            ...
            if not st.session_state.ai_background.startswith("Looks good"):
                col_insert_bg, col_dismiss_bg, _ = st.columns([2, 1, 5])
                with col_insert_bg:
        ...
        if st.session_state.smart_objective:
            ...
            if not st.session_state.smart_objective.startswith("Looks good"):
                col_insert_smart, col_dismiss_smart, _ = st.columns([2, 1, 5])
        ...
        if st.session_state.get("ai_generated_objective"):
            ...
            col_insert_obj, col_dismiss_obj, _ = st.columns([2, 1, 5])
            with col_insert_obj:
        ...
        if st.session_state.audience_profile:
            ...
            col_insert_profile, col_dismiss_profile, _ = st.columns([2, 1, 5])
        ...
        if st.session_state.get("ai_generated_audience"):
            ...
            col_insert_aud, col_dismiss_aud, _ = st.columns([2, 1, 5])
            with col_insert_aud:
    ...
    # --- Step 3: Messaging ---
        ...
        if not is_light_brief:
            ...
            if st.session_state.get("ai_insight"):
                ...
                if not str(st.session_state.ai_insight).startswith("Looks good"):
                    col_insert_insight, col_dismiss_insight, _ = st.columns([2, 1, 5])
        ...
        if st.session_state.get("ai_positioning"):
            ...
            if not str(pos.get("positioning_short", "")).startswith("Looks good"):
                col_insert_pos, col_dismiss_pos, _ = st.columns([2, 1, 5])
                with col_insert_pos:
        ...
        if st.session_state.get("ai_messages"):
            ...
            if not str(msgs[0] if isinstance(msgs, list) else msgs).startswith("Looks good"):
                col_insert_msgs, col_dismiss_msgs, _ = st.columns([2, 1, 5])
        ...
        if not is_light_brief:
            ...
            if st.session_state.get("ai_smp"):
                ...
                col_insert_smp, col_dismiss_smp, _ = st.columns([2, 1, 5])
                with col_insert_smp:
    ...
    # --- Step 4: Execution ---
        ...
        if st.session_state.get("ai_channels_proofed"):
            ...
            if not str(st.session_state.ai_channels_proofed).startswith("Looks good"):
                col_insert_ch_proof, col_dismiss_ch_proof, _ = st.columns([2, 1, 5])
        ...
        if st.session_state.get("ai_deliverables_proofed"):
            ...
            if not str(st.session_state.ai_deliverables_proofed).startswith("Looks good"):
                col_insert_del_proof, col_dismiss_del_proof, _ = st.columns([2, 1, 5])
        ...
        if st.session_state.get("ai_timeline_proofed"):
            ...
            if not str(st.session_state.ai_timeline_proofed).startswith("Looks good"):
                col_insert_tl_proof, col_dismiss_tl_proof, _ = st.columns([2, 1, 5])
    ...
    # --- Step 5: Governance ---
        ...
        if st.session_state.generated_kpis:
            ...
            col_insert_kpis, col_dismiss_kpis, _ = st.columns([2, 1, 5])
        ...
        if not is_light_brief:
            ...
            if st.session_state.generated_raci:
                ...
                col_insert_raci, col_dismiss_raci, _ = st.columns([2, 1, 5])
    ...
    <div class="app-header" style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
        <div>
            <h1 style="font-size: 2rem; font-weight: 800; color: #141d23; letter-spacing: -0.02em; margin-bottom: 4px;">
                <a href="?step=setup" target="_self" style="color: #141d23; text-decoration: none;">AI Campaign Brief Generator</a>
            </h1>
